...
static uint8_t bin2HexDigit(uint8_t bin)
{
    return (bin < 10 ? ('0' + bin) : ('A' + bin - 10));
}

static void bin2Hex8(char *data, uint8_t bin)
{
    *data     = bin2HexDigit((bin >> 4) & 0x0F);
    *(data+1) = bin2HexDigit((bin     ) & 0x0F);
}

static void bin2Hex16(char *data, uint16_t bin)
{
  bin2Hex8(data    , bin >> 8);
  bin2Hex8(data + 2, bin & 0xFF);
}

static void bin2Hex32(char *data, uint32_t bin)
{
  bin2Hex16(data    , bin >> 16);
  bin2Hex16(data + 4, bin & 0xFFFF);
}
